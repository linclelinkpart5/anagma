    ...
    P: AsRef<Path>,
{
    ...
    let mut processed_meta_file = process_meta_file(&meta_path, meta_location, meta_format, selection, sort_order)?;
    ...
    II: IntoIterator<Item = MetaLocation>,
{
    ...
    for meta_location in meta_locations.into_iter() {
        comp_mb.extend(process_item_file(&item_path, meta_location, meta_format, selection, sort_order)?);
    }
...
mod tests {
    ...
    fn test_process_meta_file() {
        ...
        for (input, expected) in inputs_and_expected {
            ...
            let produced = super::process_meta_file(meta_path, meta_location, MetaFormat::Yaml, selection, sort_order).unwrap();
    ...
    fn test_process_item_file() {
        ...
        for (input, expected) in inputs_and_expected {
            ...
            let produced = super::process_item_file(item_path, meta_location, MetaFormat::Yaml, selection, sort_order).unwrap();
            assert_eq!(expected, produced);
        }
    }
    ...
    fn test_process_item_file_flattened() {
        ...
        for (input, expected) in inputs_and_expected {
            ...
            let produced = super::process_item_file_flattened(item_path, meta_locations.clone(), MetaFormat::Yaml, selection, sort_order).unwrap();
            assert_eq!(expected, produced);
        }
    }
}
