...
pub trait FW<'p>: Iterator<Item = Result<Cow<'p, Path>, Error>> {
    fn spelunk(&mut self, selection: &Selection, sort_order: SortOrder) -> Result<(), Error>;
}
...
impl<'p> FW<'p> for ParentFileWalker<'p> {
    fn spelunk(&mut self, _selection: &Selection, _sort_order: SortOrder) -> Result<(), Error> {
        Ok(())
    }
}
...
impl<'p> FW<'p> for ChildFileWalker<'p> {
    fn spelunk(&mut self, selection: &Selection, sort_order: SortOrder) -> Result<(), Error> {
        // Manually delves into a directory, and adds its subitems to the frontier.
        if let Some(lpp) = self.last_processed_path.take() {
            // If the last processed path is a directory, add its children to the frontier.
...
impl<'p> Iterator for FileWalker<'p> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self {
            &mut Self::Parent(ref mut fw) => fw.next(),
...
mod tests {
    ...
    fn test_parent_file_walker() {
        ...
        assert_eq!(walker.next().unwrap().unwrap(), root_dir.path().join("0").join("0_1").join("0_1_0"));
        assert_eq!(walker.next().unwrap().unwrap(), root_dir.path().join("0").join("0_1"));
